...
int show_MemoryCard();//Memory_Cardフォルダの中身を一覧表示する
void check_error_fnf(int choice, int max_count);//ファイル喪失時にMemory_Cardフォルダの中身からpathを削除する


void make_MemoryCard_copy_array();//外部テキストデータを参照しその内容をコピーした配列を作成する関数
int get_target_file_size(const char* target_name);//対象のファイルのサイズを取得し、その大きさを返す関数

// int check_character_input(char base[]);//不正な内容のテキスト入力だった場合、規定の内容の値を返す関数
void test_printer(int target_size);//テスト表示用検査ツール
...
/*int check_character_input(char base[]) {//一旦封印、作業再開は後
	char temp[5];

	int target = 1;
	while (temp !='\0') {//最初の４バイトで中身がどのサイズかを判定　　　　しないと行けない。
		temp[0] = base[target];
		isalnum(base[target]);
